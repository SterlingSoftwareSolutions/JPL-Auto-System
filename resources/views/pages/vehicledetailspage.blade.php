@extends('layouts.layout')

@section('content')
    @include('components.landingpagenavbar')

    <div>
        <div>
            <section id="services" class="py-20 bg-gray-100">
                <div class="container px-6 mx-auto">
                    <div class="flex flex-wrap">
                        <div class="flex w-full px-4 mb-6 md:w-4/12">
                            <div class="p-6 bg-white rounded shadow full ">
                                <div class="overflow-x-auto">
                                    <div class="overflow-x-auto">
                                        <table class="max-w-full text-sm bg-white rounded shadow-md">
                                            <thead>

                                                <tr>
                                                    <th class="px-8 py-6">Item</th>
                                                    <th class="px-8 py-6">Vehicle Information</th>
                                                    <th class="px-8 py-6" colspan="2">Details</th>


                                                </tr>

                                                <tr>
                                                    <th class="px-4 py-2"></th>
                                                    <th class="px-4 py-2"></th>
                                                    <th class="px-4 py-2">Variant 1</th>
                                                    <th class="px-4 py-2">Variant 2</th>

                                                </tr>

                                            </thead>
                                            <tbody>
                                                <tr>

                                                    <td class="px-4 py-2 border">1</td>
                                                    <td class="px-4 py-2 border">Vehicle Make</td>
                                                    <td class="px-4 py-2 border">TBD</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">2</td>
                                                    <td class="px-4 py-2 border">Vehicle Model</td>
                                                    <td class="px-4 py-2 border">TBD</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">3</td>
                                                    <td class="px-4 py-2 border">Body Shape (NSW Body Code/Shape)</td>
                                                    <td class="px-4 py-2 border">COUPE COUPE</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">4</td>
                                                    <td class="px-4 py-2 border">Number of Side Doors</td>
                                                    <td class="px-4 py-2 border">2</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">5</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">6</td>
                                                    <td class="px-4 py-2 border">Vehicle Category</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>

                                                <tr>
                                                    <td class="px-4 py-2 border">7</td>
                                                    <td class="px-4 py-2 border">Tare Mass (kg)</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">8</td>
                                                    <td class="px-4 py-2 border">Unladen Mass (kg)</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">9</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">10</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">11</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">12</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">13</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">14</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">15</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">16</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">17</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">18</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">19</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">20</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>

                                                <tr>
                                                    <td class="px-4 py-2 border">21</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>

                                                <tr>
                                                    <td class="px-4 py-2 border">22</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">23</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>
                                                <tr>
                                                    <td class="px-4 py-2 border">24</td>
                                                    <td class="px-4 py-2 border">Number of Rear Doors</td>
                                                    <td class="px-4 py-2 border">0</td>
                                                    <td class="px-4 py-2 border">TBD</td>

                                                </tr>



                                            </tbody>
                                        </table>
                                    </div>

                                </div>

                            </div>

                        </div>
            
                        <div class="w-full px-4 mb-4 md:w-5/12">
                            <div class="p-6 bg-black rounded-lg shadow">
                                <h3 class="mb-4 text-2xl font-bold"></h3>
                                <div class="grid grid-cols-2 gap-4 mb-4">
                                    <img src="{{ asset('images/fig-1.png') }}" alt="fig 1"
                                        class="w-full rounded shadow">
                                    <img src="{{ asset('images/fig-2.png') }}" alt="fig 2"
                                        class="w-full rounded shadow">

                                </div>
                                <h3 class="mb-4 text-sm text-center text-white">Fig 3 Dashboard / Interior view</h3>


                                <div class="grid grid-cols-3 gap-4 mb-4">
                                    <div class="col-span-1"></div>
                                    <img src="{{ asset('images/fig-3.png') }}" alt="fig 3"
                                        class="w-full rounded shadow">
                                    <div class="col-span-1"></div>
                                </div>
                                <h3 class="mb-4 text-sm text-center text-white">Fig 1 and Fig 2 Front and rear view

                                </h3>

                                <div class="grid grid-cols-2 gap-4 mb-4">
                                    <img src="{{ asset('images/fig-4.png') }}" alt="fig 4"
                                        class="w-full rounded shadow">
                                    <img src="{{ asset('images/fig-5.png') }}" alt="fig 5"
                                        class="w-full rounded shadow">

                                </div>
                                <h3 class="mb-4 text-sm text-center text-white">Fig 4 and Fig 5. Engine bay view and
                                    underbody view

                                </h3>

                            </div>
                        </div>



                        <div class="flex justify-end w-full px-4 mb-6 md:w-3/12">
                            <div class="p-6 bg-white rounded shadow">
                                <h3 class="mb-4 text-2xl font-bold">Measurement</h3>
                                <p class="text-gray-600">Lorem ipsum dolor sit amet, consectetur adipiscing elit,
                                    sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim
                                    ad minim veniam, quis nostrud
                                    exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
                                <img src="{{ asset('images/measurement.png') }}" alt="fig 5"
                                    class="w-full rounded shadow">
                            </div>
                        </div>


                    </div>
                </div>
            </section>

        </div>

        <!--------------------Second div---------------------------------------------------------->
        <div>


            <div>
                
                <section class="py-8">
                    <div class="flex items-center mb-4">
                        <h1 class="text-2xl font-bold">Specifications</h1>
                    </div>
                    <input type="text" id="searchInput" placeholder="Search Specific attribute..." class="flex justify-center p-2 mb-4 border border-gray-300 rounded item-center">
                    

                    <div class="flex items-center justify-between mb-10 text-sm text-gray-600 md-[80%]">
                        
                        
                        <p><span class="font-bold">Explanation</span></p>

                        <p><span class="font-bold">Included</span></p>
                        <p><span class="font-bold">Unavailable</span></p>
                        <p><span class="font-bold">Optional</span></p>
                        <p><span class="font-bold">Information is missing</span></p>
                    </div>

                    <nav class="flex items-center justify-center border-b border-gray-300">
                        <a href="#" onclick="showTab('engine')" class="inline-block px-4 py-2 text-gray-600 hover:text-blue-500 focus:text-blue-500">Engine & Performance</a>
                        <a href="#" onclick="showTab('dimensions')" class="inline-block px-4 py-2 text-gray-600 hover:text-blue-500 focus:text-blue-500">Dimensions & Weight</a>
                        <a href="#" onclick="showTab('exterior')" class="inline-block px-4 py-2 text-gray-600 hover:text-blue-500 focus:text-blue-500">Exterior</a>
                        <a href="#" onclick="showTab('interior')" class="inline-block px-4 py-2 text-gray-600 hover:text-blue-500 focus:text-blue-500">Interior</a>
                        <a href="#" onclick="showTab('features')" class="inline-block px-4 py-2 text-gray-600 hover:text-blue-500 focus:text-blue-500">Features</a>
                        <a href="#" onclick="showTab('safety')" class="inline-block px-4 py-2 text-gray-600 hover:text-blue-500 focus:text-blue-500">Safety & Security</a>
                        <a href="#" onclick="showTab('other')" class="inline-block px-4 py-2 text-gray-600 hover:text-blue-500 focus:text-blue-500">Other</a>
                    </nav>
            
                    <div class="w-full max-w-5xl mx-auto overflow-hidden bg-white rounded-lg shadow-md">
                        <div id="engine" class="hidden tab-content">
                            <div class="flex">
                                <div class="w-1/3">
                                    <img src="{{ asset('images/dimensions-and-weight.png') }}" alt="fig 5" class="w-full rounded shadow">
                                </div>
                                
                                

                                <div class="w-1/2 p-4">
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            <h3 class="mb-6 text-lg font-bold">Exterior</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Length</span><span class="border-b-2 border-black">4,612 mm</span></li>
                                                <li class="flex justify-between"><span>Width</span><span class="border-b-2 border-black">1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Width, with Mirrors</span><span class="border-b-2 border-black">1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Height</span><span class="border-b-2 border-black">1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Track, Front</span><span class="border-b-2 border-black">1,513 mm</span></li>
                                                <li class="flex justify-between"><span>Track, Rear</span><span class="border-b-2 border-black">1,507 - 1,513 mm</span></li>
                                                <li class="flex justify-between"><span>Wheel Base</span><span class="border-b-2 border-black">2,760 mm</span></li>
                                                <li class="flex justify-between"><span>Overhang, Front</span><span class="border-b-2 border-black">2,760 mm</span></li>
                                                <li class="flex justify-between"><span>Overhang, Rear</span><span class="border-b-2 border-black">2,760 mm</span></li>
                                            </ul>
                                        </div>
                                        <div>
                                            <h3 class="mb-6 text-lg font-bold">Interior</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Head Room, Front</span><span>4,612 mm</span></li>
                                                <li class="flex justify-between"><span>Head Room, Rear</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Hip Room, Front</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Hip Room, Rear</span><span>1,782 mm</span></li>
                                            </ul>
                                            <h3 class="mt-4 mb-6 text-lg font-bold">Weight</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Curb Weight</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Gross Weight</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Trailer Load, braked 12%</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Trailer Load, Unbraked</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Cargo Capacity</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Towbar Download</span><span>75 kg</span></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div id="dimensions" class="hidden tab-content">
                            <!-- Dimensions & Weight content -->
                            <div class="flex">
                                <div class="w-1/3">
                                    <img src="{{ asset('images/dimensions-and-weight.png') }}" alt="fig 5" class="w-full rounded shadow">
                                </div>
                                <div class="w-1/2 p-4">
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            <h3 class="mb-6 text-lg font-bold">Exterior</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Length</span><span>4,612 mm</span></li>
                                                <li class="flex justify-between"><span>Width</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Width, with Mirrors</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Height</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Track, Front</span><span>1,513 mm</span></li>
                                                <li class="flex justify-between"><span>Track, Rear</span><span>1,507 - 1,513 mm</span></li>
                                                <li class="flex justify-between"><span>Wheel Base</span><span>2,760 mm</span></li>
                                                <li class="flex justify-between"><span>Overhang, Front</span><span>2,760 mm</span></li>
                                                <li class="flex justify-between"><span>Overhang, Rear</span><span>2,760 mm</span></li>
                                            </ul>
                                        </div>
                                        <div>
                                            <h3 class="mb-6 text-lg font-bold">Interior</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Head Room, Front</span><span>4,612 mm</span></li>
                                                <li class="flex justify-between"><span>Head Room, Rear</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Hip Room, Front</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Hip Room, Rear</span><span>1,782 mm</span></li>
                                            </ul>
                                            <h3 class="mt-4 mb-6 text-lg font-bold">Weight</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Curb Weight</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Gross Weight</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Trailer Load, braked 12%</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Trailer Load, Unbraked</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Cargo Capacity</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Towbar Download</span><span>75 kg</span></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div id="exterior" class="hidden tab-content">
                            <div class="flex">
                                <div class="w-1/3">
                                    <img src="{{ asset('images/dimensions-and-weight.png') }}" alt="fig 5" class="w-full rounded shadow">
                                </div>
                                <div class="w-1/2 p-4">
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            <h3 class="mb-6 text-lg font-bold">Exterior</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Length</span><span>4,612 mm</span></li>
                                                <li class="flex justify-between"><span>Width</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Width, with Mirrors</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Height</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Track, Front</span><span>1,513 mm</span></li>
                                                <li class="flex justify-between"><span>Track, Rear</span><span>1,507 - 1,513 mm</span></li>
                                                <li class="flex justify-between"><span>Wheel Base</span><span>2,760 mm</span></li>
                                                <li class="flex justify-between"><span>Overhang, Front</span><span>2,760 mm</span></li>
                                                <li class="flex justify-between"><span>Overhang, Rear</span><span>2,760 mm</span></li>
                                            </ul>
                                        </div>
                                        <div>
                                            <h3 class="mb-6 text-lg font-bold">Interior</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Head Room, Front</span><span>4,612 mm</span></li>
                                                <li class="flex justify-between"><span>Head Room, Rear</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Hip Room, Front</span><span>1,782 mm</span></li>
                                                <li class="flex justify-between"><span>Hip Room, Rear</span><span>1,782 mm</span></li>
                                            </ul>
                                            <h3 class="mt-4 mb-6 text-lg font-bold">Weight</h3>
                                            <ul class="text-sm">
                                                <li class="flex justify-between"><span>Curb Weight</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Gross Weight</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Trailer Load, braked 12%</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Trailer Load, Unbraked</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Cargo Capacity</span><span>1,670 - 1,905 kg</span></li>
                                                <li class="flex justify-between"><span>Max Towbar Download</span><span>75 kg</span></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div id="interior" class="hidden tab-content">
                            <!-- Interior content -->
                            <p>Interior details go here...</p>
                        </div>
                        <div id="features" class="hidden tab-content">
                            <!-- Features content -->
                            <p>Features details go here...</p>
                        </div>
                        <div id="safety" class="hidden tab-content">
                            <!-- Safety & Security content -->
                            <p>Safety & Security details go here...</p>
                        </div>
                        <div id="other" class="hidden tab-content">
                            <!-- Other content -->
                            <p>Other details go here...</p>
                        </div>
                    </div>
                </section>
            
                <script>
                    function showTab(tabId) {
                        // Hide all tab contents
                        const tabs = document.querySelectorAll('.tab-content');
                        tabs.forEach(tab => {
                            tab.classList.add('hidden');
                        });
            
                        // Show the selected tab content
                        const selectedTab = document.getElementById(tabId);
                        selectedTab.classList.remove('hidden');
            
                        // Remove active class from all links
                        const links = document.querySelectorAll('nav a');
                        links.forEach(link => {
                            link.classList.remove('text-blue-500');
                            link.classList.add('text-gray-600');
                        });
            
                        // Add active class to the clicked link
                        const activeLink = Array.from(links).find(link => link.getAttribute('onclick').includes(tabId));
                        activeLink.classList.remove('text-gray-600');
                        activeLink.classList.add('text-blue-500');
                    }
            
                    // Show default tab
                    showTab('engine');
                </script>
            </div>
            
        </div>

    </div>
 
@endsection
